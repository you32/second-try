...
YOLO_STRIDES_TINY = [16, 32]
YOLO_ANCHORS_TINY = [23,27, 37,58, 81,82, 81,82, 135,169, 344,319]
YOLO_XYSCALE_TINY = [1.05, 1.05]
YOLO_STRIDES = [8, 16, 32]
YOLO_ANCHORS = [12,16, 19,36, 40,28, 36,75, 76,55, 72,146, 142,110, 192,243, 459,401]
YOLO_ANCHORS_V3 = [10,13, 16,30, 33,23, 30,61, 62,45, 59,119, 116,90, 156,198, 373,326]
...
def load_config(FLAGS):
    if FLAGS.tiny:
        STRIDES = np.array(YOLO_STRIDES_TINY)
        ANCHORS = get_anchors(YOLO_ANCHORS_TINY, FLAGS.tiny)
        XYSCALE = YOLO_XYSCALE_TINY if FLAGS.model == 'yolov4' else [1, 1]
    else:
        STRIDES = np.array(YOLO_STRIDES)
        if FLAGS.model == 'yolov4':
            ANCHORS = get_anchors(YOLO_ANCHORS, FLAGS.tiny)
        elif FLAGS.model == 'yolov3':
            ANCHORS = get_anchors(YOLO_ANCHORS_V3, FLAGS.tiny)
        XYSCALE = YOLO_XYSCALE if FLAGS.model == 'yolov4' else [1, 1, 1]
...
def draw_bbox(image, bboxes, classes=read_class_names(YOLO_CLASSES), allowed_classes=list(read_class_names(YOLO_CLASSES).values()), show_label=True):
